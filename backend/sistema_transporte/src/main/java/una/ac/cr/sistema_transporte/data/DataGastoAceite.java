...
public class DataGastoAceite extends DataBase {
    ...
    public boolean agregarRelleno(RellenoAceite relleno) {
        try {
            Connection cn = getConexion();
            String query = "INSERT INTO relleno_aceite (mantenimiento, cantidad, observaciones, fecha) VALUES (?, ?, ?, ?)";
            PreparedStatement preparedStatement = cn.prepareStatement(query);
            ...
            preparedStatement.setDate(4, new java.sql.Date(relleno.getFecha().getTime()));
            preparedStatement.executeUpdate();
            return true;
        } catch (SQLException e) {
            System.out.println("\n  error encontrado : " + e.toString());
